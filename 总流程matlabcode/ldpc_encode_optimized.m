function [codeout,H,Z_c,encoded_bits_original] = ldpc_encode_optimized(s,base_graph_index)
%% 
K = length(s);
...
H=double(H);% 0328增加 脚本调用时定义类型
codeout=double(encoded_bits);% 0328增加 脚本调用时定义类型
